...
public class InAirState : IState
{
    ...
    {
        if (player.IsGrounded && player.Movement.XVelocity == 0 && player.Movement.YVelocity == 0)
            return player.StateMachine.IdleState;
        if (player.IsGrounded && player.Movement.IsSprinting && player.Movement.YVelocity == 0)
